...
namespace GmtkJam2019.Physics
{
	public class HybridWorld
	{
		...
		private List<HybridBody> staticBodies;
		private List<HybridBody> dynamicBodies;

		private float accumulator;

		public HybridWorld()
		{
			staticBodies = new List<HybridBody>();
			dynamicBodies = new List<HybridBody>();
		}

		public List<HybridBody> StaticBodies => staticBodies;
		public List<HybridBody> DynamicBodies => dynamicBodies;
		...
		public void Remove(HybridBody body)
		{
		}
		...
		private void Solve(float step)
		{
			foreach (var dynamicBody in dynamicBodies)
			{
				...
				{
					vec2 v = MergeVectors(correctionList);
					dynamicBody.Position += new vec3(v.x, 0, v.y);
				}
			}
		}

		private bool ProcessBodies(HybridBody dynamicBody, HybridBody staticBody, out vec2 v)
		{
			v = vec2.Zero;

			float delta = Math.Abs(dynamicBody.Position.y - staticBody.Position.y);
